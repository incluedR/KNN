...
if __name__ == "__main__":
    ...
    for i in range(0,groupnum):
        ...
        trainLabel_cut= np.array_split(trainLabel,groupnum)[i]
        if(i == 0):
            trainData = trainData[3:num]
            trainLabel = trainLabel[3:num]
        elif(i == groupnum-1):
            trainData = trainData[0:num-3]
            trainLabel = trainLabel[0:num-3]
        else:
            trainData = np.vstack((trainData[3+3*i:num-3],trainData[0:i*3]))
            trainLabel = np.vstack((trainLabel[3+3*i:num-3],trainLabel[0:i*3]))
        print(trainData)
        dists = euclideanDistance_two_loops(trainData, trainData_cut)
        y_pred = predict_labels(dists, trainLabel, k=5)
        deletelist = []
