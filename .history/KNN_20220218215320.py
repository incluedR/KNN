...
def load_data_forscatter_after(New_traindata,New_trainlabel):
    man = []
    famale =[]
    for i in range(0,len(New_trainlabel)):
        if(New_trainlabel[i]=='男'): 
            man.append(New_traindata[i])
        else:
            famale.append(New_traindata[i])
...
def plot_distrbution_after(trainData,trainLabel):
    man,famale = load_data_forscatter_after(trainData,trainLabel)
    import matplotlib.pyplot as plt
    plt.rcParams['font.sans-serif'] = ['SimHei']  # 黑体，为了显示中文
    plt.rcParams['font.size'] = '12'  # 12号，增大字体
    fig = plt.figure()
...
def clipping_KNN(membernum):
    ...
    New_trainlabel = np.array(New_trainlabel)
...
if __name__ == "__main__":
    ...
    acc_array = []
    New_traindata , New_trainlabel = clipping_KNN(12)
    plot_distrbution_after(New_traindata, New_trainlabel)

    dists = euclideanDistance_two_loops(New_traindata, testData)
    #y_pred = predict_labels(dists, New_trainlabel, k=5)
    #accuracy = getAccuracy(y_pred, testLabel)
    #print("K值为5"+",准确率为： "+str(accuracy))
    print(New_trainlabel)
